...
var sum = JSON.parse(sessionStorage.getItem("headcount"));//用 JSON.parse(arr) 转出字符串
...
//动态设置内容
    ...
    $(".knife").click(function(){
        ...
        if(players[index].role !== "杀手" && players[index].state === "live"){//判断选择杀死的人是不是“杀手”，不是则执行下一步
            if(confirm("确定要杀他吗？") === true) {
                ...
                for( i = 0;i<players.length;i++){
                    ...
                    else if(players[i].state==="live" && players[i].role==="杀手"){
    ...
    $(".footer-btn-q").click(function(){
        if(index === -1){
            ...
            if(killers.length >= civilians.length){
                ...
                alert("杀手获得胜利");
                window.location.href = "js-2(result).html";
            }
